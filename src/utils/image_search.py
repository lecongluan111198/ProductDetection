...
class Search_Setup:
    ...
    def __init__(self, image_list: list):
        ...
        os.makedirs(f'metadata-files/{self.model_name}', exist_ok=True)
        print("\033[91m Please Wait Model Is Loading or Downloading From Server!")
        base_model = models.vgg19()
        base_model.load_state_dict(torch.load('resources/weights/vgg19/vgg19.pth'))
    ...
    def _search_by_vector_v2(self, v, n: int):
        self.v = v
        self.n = n
        index = faiss.read_index(image_features_vectors_idx(self.model_name))
        D, I = index.search(np.array([self.v], dtype=np.float32), self.n)
        return dict(zip(D[0], self.image_data.iloc[I[0]]['images_paths'].to_list()))
    ...
    def get_similar_images(self, image_path: str, number_of_images: int = 10):
        ...
        img_dict = self._search_by_vector_v2(query_vector, self.number_of_images)
        return img_dict
